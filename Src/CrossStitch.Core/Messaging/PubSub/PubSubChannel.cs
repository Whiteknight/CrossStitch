...
namespace CrossStitch.Core.Messaging.PubSub
{
    public class PubSubChannel<TPayload> : IPubSubChannel<TPayload>
    {
        ...
        public SubscriptionToken Subscribe(Action<TPayload> act, Func<TPayload, bool> filter, PublishOptions options)
        {
            var id = Guid.NewGuid();
        ...
        private IPubSubSubscription<TPayload> CreateSubscription(Action<TPayload> act, Func<TPayload, bool> filter, PublishOptions options)
        {
